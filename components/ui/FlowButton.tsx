  ...
  return (
    <>
      ...
      <button
        style={
          {
            ["--ring-color" as any]: "rgba(56,189,248,0.95)",
          } as React.CSSProperties
        }
        className={`
          flowbtn group relative flex items-center gap-1 overflow-hidden
          rounded-lg border-2 border-cyan-400 bg-gradient-to-r from-cyan-500 to-blue-600
          px-10 py-6 text-lg font-bold text-white cursor-pointer
          shadow-xl shadow-cyan-500/30
          transition-all duration-[600ms] ease-[cubic-bezier(0.23,1,0.32,1)]
          hover:border-cyan-300 hover:shadow-cyan-400/50 hover:-translate-y-1
          active:scale-[0.95]
          ${className}
        `}
      >
        ...
        {/* Left arrow (arr-2) */}
        <ArrowRight
          className="absolute w-5 h-5 left-[-25%] stroke-white stroke-[2.5] fill-none z-[9]
                     group-hover:left-4
        ...
        {/* Circle (inverted stage: expands cyan on hover) */}
        <span
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2
                     w-4 h-4 bg-gradient-to-r from-cyan-300 to-sky-200 rounded-[50%] opacity-0
                     group-hover:w-[280px] group-hover:h-[280px] group-hover:opacity-100
                     transition-all duration-[800ms] ease-[cubic-bezier(0.19,1,0.22,1)]"
        />

        {/* Right arrow (arr-1) */}
        <ArrowRight
          className="absolute w-5 h-5 right-4 stroke-white stroke-[2.5] fill-none z-[9]
